...
public final class IPCClient implements Closeable {
	...
	public void close() {
		...
		try {
			...
			if (debugMode) {
				System.out.printf("Failed to close pipe: %s%n", e);
			}
		}
	}
		...
		readThread = new Thread(() -> {
			try {
					...
					if (json != null) {
						...
						if (listener != null && json.has("cmd") && json.getAsJsonPrimitive("cmd").getAsString().equals("DISPATCH")) {
							try {
								...
								System.out.printf("Exception when handling event: %s%n", e);
							}
						}
					}
				}
				...
				if (ex instanceof IOException)
					System.out.printf("Reading thread encountered an IOException: %s%n", ex);
				else
					System.out.printf("Reading thread encountered an JSONException: %s%n", ex);
